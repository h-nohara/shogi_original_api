...
function get_legal_moves(Board, loc=null){
    ...
    let natural_moves = get_all_natural_moves(Board);
    ...
    for (move of natural_moves){
        ...
        // 駒を移動
        let board_copy = deepcopy_Board(Board);
        board_copy.all_pieces["main"][move.slice(0, 2)] = null;
        let new_piece = new Piece(original_piece.name, original_piece.is_sente);
        new_piece.loc = move.slice(2, 4);
        board_copy.all_pieces["main"][move.slice(2, 4)] = new_piece;
...
function get_all_natural_moves(Board, loc=null){
    ...
    if (loc == null){
        ...
        if (is_sente){
            var pieces_in_hand = Board.all_pieces["h&&"]["sente"];
        }
        else{
            var pieces_in_hand = Board.all_pieces["h&&"]["gote"];
        }

        for (piece_name of Object.keys(pieces_in_hand)){
            if (pieces_in_h&&[piece_name] > 0){
        ...
        // 移動と打ち手を合わせる
        var all_natural_moves = natural_moves_move + natural_moves_put;
...
function get_natural_moves_move(Piece, Board){
    ...
    let potential_dests = [];  // この駒が本来動ける方向
